...
const Orders = () => {
  ...
  const totalSales = data.reduce((acc, order) => acc + Number(order.TotalPrice || 0), 0);
          ...
          <div className="bg-white rounded-lg shadow-lg w-[500px] max-h-[80vh] overflow-y-auto p-6">
            ...
            <div className="mb-4 space-y-1">
              {[
                ...
                { label: 'Order Confirm', value: selectedOrder.orderConfirm ? 'Yes' : 'No' },
              ].map((field, index) => (
                <OrderDetailItem key={index} label={field.label} value={field.value} />
              ))}
            </div>
                ...
                {selectedOrder.products.map((item, idx) => (
                  <li key={idx}>
                    <div className="grid grid-cols-3 sm:grid-cols-4 items-center text-sm">
                      ...
                      <div className="text-right">Price: ₹{item.productPrice}</div>
                    </div>
                  </li>
                ))}
              </ul>
