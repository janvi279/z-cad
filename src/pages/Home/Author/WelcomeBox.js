  ...
  const fetchSalesData = async () => {
    ...
    try {
      ...
      if (lastWithdrawalDate) {
        ...
        const allowed = currentDate.getMonth() !== last.getMonth() || currentDate.getFullYear() !== last.getFullYear();
    ...
    <div className='p-6 space-y-6'>
      ...
      <div className='flex items-start gap-4'>
        <div className='h-24 w-24 rounded-full bg-gray-200 flex items-center justify-center'>
          {profileData?.avtar ? (
            <img src={profileData.avtar} alt='Profile' className='h-24 w-24 rounded-full object-cover' />
          ) : (
            <FaRegUser className='h-8 w-8 text-gray-500' />
          )}
        </div>
        <div className='space-y-1'>
          <h1 className='text-2xl font-medium text-sky-500'>Welcome to the ZCAD PUBLICATION Dashboard</h1>
      ...
      <div className='grid gap-4 md:grid-cols-3'>
        ...
        <div className='flex shadow-md rounded-lg overflow-hidden'>
          ...
          <div className='bg-white p-4 flex-1'>
            <p className='text-lg font-medium'>{salesData.ordersReceived} orders</p>
