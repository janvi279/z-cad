  ...
  const fetchSalesData = async () => {
    try {
      ...
      const filteredOrders = orders.filter(order => {
        const orderDate = new Date(order.orderCreate) // Changed to orderCreate
        return orderDate >= fromDate && orderDate <= toDate
      })

      const filteredRefunds = refunds.filter(refund => {
        const refundDate = new Date(refund.createdAtShopify) // Changed to createdAt
        return refundDate >= fromDate && refundDate <= toDate
      })

      const filteredWithdrawals = withdrawals.filter(transaction => {
        const withdrawalDate = new Date(transaction.date) // Changed to date
      ...
      filteredOrders.forEach(order => {
        const orderDate = new Date(order.orderCreate) // Changed to orderCreate
        const value = parseFloat(order.TotalPrice) // Changed to TotalPrice
        if (!isNaN(value)) {
          grossSales += value
          ordersPlaced += 1
          itemsPurchased += order.products.reduce((sum, product) => sum + product.quantity, 0) // Changed to products
      ...
      filteredRefunds.forEach(refund => {
        refund.transactions.forEach(transaction => {
          totalRefund += parseFloat(transaction.amount) || 0;
        });
      })
